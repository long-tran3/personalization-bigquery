...
--         ARRAY(select productSKU as product from unnest(hits.product)) as product,
        ...
        products.productListPosition as position
...
_table_suffix between '20200110' and '20200203' 
and hits.eCommerceAction.action_type in  ('2', '3', '6') 
and safe_cast(products.productSKU as INT64) is not null
),
...
flatten_data_join_main_category as (
    ...
    WHERE
        ...
        cc.children_count > 0 AND (d.action != '2' or (d.action = '2' and d.position = 0))
),
...
select num_prods as product_interact_in_session, count(*) as num_session from final group by num_prods order by num_prods
-- select num_main_category as main_category_interact_in_session , count(*) as num_session 
-- from final group by main_category_interact_in_session order by main_category_interact_in_session

-- select num_primary_category as primary_category_interact_in_session , count(*) as num_session from final 
-- group by primary_category_interact_in_session order by primary_category_interact_in_session
